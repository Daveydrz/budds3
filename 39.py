...
def enhanced_speaker_check(audio_file_path):
    ...
    try:
        ...
        for i, segment in enumerate(segments_to_try):
            ...
            speaker, confidence = detect_active_speaker(segment, require_confidence=0.85)
...
def parallel_interrupt_detector():
    """Fixed parallel interrupt detector - less aggressive"""
    import pyaudio
    import numpy as np
    ...
    try:
        ...
        required_loud_frames = 5  # ✅ INCREASED - need more frames to trigger
        ...
        while True:
            try:
                ...
                # ✅ LONGER COOLDOWN - prevent repeated interrupts
                if current_time - last_interrupt_time < 8.0:  # 8 second cooldown
                    time.sleep(0.2)
                    continue
                
                # Only detect when Buddy is actually talking
                if not buddy_talking.is_set():
                    time.sleep(0.2)
                    consecutive_loud_frames = 0
                    continue
                ...
                # ✅ HIGHER THRESHOLDS - require louder sound
                is_loud = rms_volume > 2500 and peak_volume > 6000  # Much higher
                
                if is_loud:
                    consecutive_loud_frames += 1
                    if consecutive_loud_frames == 1:
                        print(f"[Buddy][Parallel] 🔊 Loud audio: RMS:{rms_volume} PEAK:{peak_volume}")
                    
                    # ✅ REQUIRE MORE FRAMES before triggering
                    if consecutive_loud_frames >= required_loud_frames:
                        print(f"[Buddy][Parallel] 🚨 INTERRUPT DETECTED! ({consecutive_loud_frames} frames)")
                        ...
                        if buddy_talking.is_set():
                            ...
                            consecutive_loud_frames = 0
                            print("[Buddy][Parallel] ⏳ Entering 8-second cooldown...")
                        else:
                            consecutive_loud_frames = 0
                else:
        ...
        time.sleep(2)
        parallel_interrupt_detector()  # Restart
    finally:
        try:
            if stream:
                stream.stop_stream()
...
# ========== VAD + LISTEN ==========
def vad_and_listen():
    """Properly fixed VAD with accurate speech end detection"""
    vad = webrtcvad.Vad(2)  # Medium sensitivity
    blocksize = int(MIC_SAMPLE_RATE * 0.02)  # 20ms blocks
    
    # ✅ PROPER THRESHOLDS for natural speech
    min_speech_frames = 4       
    silence_thresh = 1.0        # 1 second of silence to stop
    max_recording_time = 8      # Reasonable max time
    min_recording_time = 0.5    # Minimum to avoid cutting off
        ...
        while True:
            try:
                ...
                for i in range(0, len(mic_16k), 160):
                    ...
                    # Show activity indicator less frequently
                    if total_frames_processed % 50 == 0:
                        print(f"[🎤] Listening... (vol: {volume:.0f})", end='\r')
                    ...
                    if is_speech:
                        ...
                        # Start recording when confident
                        if speech_detected >= min_speech_frames:
                            ...
                            # ✅ FIXED: Proper silence detection during recording
                            recording_frames = 0
                            ...
                            while (time.time() - start_time) < max_recording_time:
                                try:
                                    ...
                                    for j in range(0, len(mic_16k), 160):
                                        ...
                                        # ✅ PROPER silence tracking
                                        if volume2 < 250:  # Silence threshold
                                        ...
                                        # ✅ FIXED: Stop when silence_thresh is reached
                                        elapsed_recording = time.time() - start_time
                                        time_since_last_speech = time.time() - last_speech_time
                                        
                                        # Stop conditions
                                        if elapsed_recording >= min_recording_time:
                                            if time_since_last_speech >= silence_thresh:
                                                print(f"\n[VAD] ✅ Speech ended - stopping after {elapsed_recording:.1f}s")
                                                break
                                    ...
                                    # Break outer loop when silence detected
                                    elapsed_recording = time.time() - start_time
                                    time_since_last_speech = time.time() - last_speech_time
                                    if elapsed_recording >= min_recording_time and time_since_last_speech >= silence_thresh:
                                        break
...
def main():
    ...
    # Start parallel interrupt system
    print("[Buddy] 🚀 Starting parallel interrupt system...")
    threading.Thread(target=parallel_interrupt_detector, daemon=True).start()
    print("[Buddy] ✅ Parallel interrupt system active!")
    
    # ✅ START STANDBY MONITORING
    print("[Buddy] 🚀 Starting auto-standby monitor...")
    threading.Thread(target=auto_standby_monitor, daemon=True).start()
...
def handle_user_interaction(speaker, history, conversation_mode="auto", user_input=None):
    ...
    # Time/date detection with CURRENT datetime
...
def main():
    ...
    # ✅ REMOVED AUTO-STANDBY - it was causing immediate shutdowns
    
    in_session, session_timeout = False, 60  # Longer session timeout
    speaker = None
    history = []
    last_time = 0
    ...
    try:
        while True:
            if not in_session:
                ...
                if time.time() - last_time > session_timeout:
                    if DEBUG:
                        print("[Buddy] Session expired - returning to wake word mode.")
                    in_session = False
                    continue
