...
def tts_worker():
    ...
    while True:
        try:
            ...
            # PATCH: Abort TTS if barge-in occurred
            if full_duplex_interrupt_flag.is_set():
                print("[Buddy][TTS_WORKER] Interrupt flag set, skipping TTS.")
                tts_queue.task_done()
                continue
            ...
            # PATCH: Check interrupt again before generating audio
            if full_duplex_interrupt_flag.is_set():
                print("[Buddy][TTS_WORKER] Interrupt flag set before TTS, skipping.")
                tts_queue.task_done()
                continue
...
def audio_playback_worker():
    ...
    while True:
        try:
            ...
            # PATCH: Check interrupt flag before processing chunk
            if full_duplex_interrupt_flag.is_set():
                if current_audio_playback and current_audio_playback.is_playing():
                    current_audio_playback.stop()
                    current_audio_playback = None
                # PATCH: Clear all queued audio!
            ...
            # PATCH: Check interrupt flag again before playing
            if full_duplex_interrupt_flag.is_set():
                if current_audio_playback and current_audio_playback.is_playing():
                    current_audio_playback.stop()
                    current_audio_playback = None
    ...
    try:
        ...
        with playback_lock:
            try:
                ...
                # PATCH: Start VAD monitoring if not already running
                if not buddy_talking.is_set():
                    buddy_talking.set()
                    if not vad_thread_active.is_set():
                        threading.Thread(target=background_vad_listener, daemon=True).start()

                # PATCH: Start reference audio for AEC
                threading.Thread(
                    target=update_reference_audio_realtime, 
                    args=(smooth_audio, target_sr), 
                    daemon=True
                ...
                # PATCH: Abort if user interrupted before playback
                if full_duplex_interrupt_flag.is_set():
                    if DEBUG:
                ...
                # PATCH: Tight monitoring loop for instant barge-in
                while current_audio_playback.is_playing():
                    if full_duplex_interrupt_flag.is_set():
                        if DEBUG:
                ...
                # PATCH: Clear talking flag if playback queue is empty
                if playback_queue.empty():
                    buddy_talking.clear()
...
def stop_playback():
    ...
    # PATCH: Set interrupt flag first
    full_duplex_interrupt_flag.set()

    # PATCH: Stop current playback instantly
    try:
        with playback_lock:
            if current_audio_playback and current_audio_playback.is_playing():
                current_audio_playback.stop()
    ...
    # PATCH: Clear BOTH playback and TTS queues quickly
    cleared_playback = 0
    while not playback_queue.empty():
        try:
            playback_queue.get_nowait()
            ...
            break
    if DEBUG and cleared_playback > 0:
        print(f"[Buddy] 🗑️ Cleared {cleared_playback} queued playback chunks")
    ...
    while not tts_queue.empty():
        try:
            ...
            break
    if DEBUG and cleared_tts > 0:
        print(f"[Buddy] 🗑️ Cleared {cleared_tts} queued TTS chunks")

    # PATCH: Also clear buffered audio if using a buffer
    global audio_buffer
    if 'audio_buffer' in globals():
        import collections
        audio_buffer = collections.deque()
        if DEBUG:
            print("[Buddy] 🗑️ Cleared audio_buffer")
...
def ask_llama3_openai_streaming(messages, model="llama3", max_tokens=80, temperature=0.6, lang="en", style=None):
    ...
    try:
        ...
        def speak_chunk_natural(chunk_text, stream_id):
            ...
            try:
                pcm, sr = generate_kokoro_pcm(chunk_text, lang=lang, style=tts_style)
                if pcm is not None and sr:
                    playback_queue.put((pcm, sr))
